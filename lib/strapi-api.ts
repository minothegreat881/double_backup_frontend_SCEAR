...
export async function uploadGalleryPhoto(photoData: {
  ...
  try {
    ...
    console.log('🔥 RAW UPLOAD: Using raw-upload endpoint to prevent rotation...');
...
export async function deleteGalleryPhoto(photoId: number | string): Promise<boolean> {
  try {
    console.log('🗑️ Deleting gallery photo:', photoId);
    ...
    if (response.status === 404 && typeof photoId === 'string') {
      console.log('🔄 Retrying delete with numeric ID lookup...');
      ...
      if (fetchResponse.ok) {
        ...
        if (photo && photo.id) {
          console.log(`🎯 Found numeric ID ${photo.id} for documentId ${photoId}`);
...
export async function updateGalleryPhoto(photoId: number | string, updateData: Partial<{
  ...
  try {
    console.log('📝 Updating gallery photo:', photoId, updateData);
    ...
    if (response.status === 404) {
      console.log('🔄 Retrying update with ID lookup...');
      ...
      if (fetchResponse.ok) {
        ...
        if (photo && photo.id) {
          console.log(`🎯 Found photo with ID ${photo.id} for requested ${photoId}`);
          ...
          console.log('❌ Photo not found in Strapi data');
        }
      }
    }
...
export async function createEvent(eventData: {
  ...
  try {
    ...
    if (eventData.file) {
      ...
      console.log('🔥 EVENT RAW UPLOAD: File size =', eventData.file.size, 'name =', eventData.file.name);
      const fileResponse = await fetch(`${STRAPI_BASE_URL}/api/raw-upload`, {
        method: 'POST',
        body: fileFormData,
      });
      ...
      const uploadedFiles = await fileResponse.json();
      console.log('🎯 EVENT photo uploaded SUCCESS:', uploadedFiles?.length, 'files, first ID =', uploadedFiles?.[0]?.id);
...
export async function updateEvent(eventId: number | string, updateData: Partial<{
  ...
  try {
    ...
    if (updateData.file) {
      ...
      console.log('🔥 RAW UPLOAD: Using raw-upload endpoint for update...');
      const fileResponse = await fetch(`${STRAPI_BASE_URL}/api/raw-upload`, {
        method: 'POST',
        body: fileFormData,
      });
      ...
      return [
        {
          ...
          title: 'Archeologický výskum - Carnuntum',
          description: 'Objavte tajomstvá rímskej histórie v Carnuntume, kde sa stretávate s archeologickými nálezmi a rímskou kultúrou.',
        ...
        },
        {
          ...
          title: 'Rímsky vojenský tábor - Vindobona',
          description: 'Navštívte zvyšky rímskeho vojenského tábora vo Viedni a spoznajte život rímskych legionárov.',
...
export async function createActivity(activityData: {
  ...
  try {
    ...
    if (activityData.file) {
      ...
      console.log('🔥 ACTIVITY RAW UPLOAD: File size =', activityData.file.size, 'name =', activityData.file.name);
      const fileResponse = await fetch(`${STRAPI_BASE_URL}/api/raw-upload`, {
        method: 'POST',
        body: fileFormData,
      });
      ...
      const uploadedFiles = await fileResponse.json();
      console.log('🎯 ACTIVITY photo uploaded SUCCESS:', uploadedFiles?.length, 'files, first ID =', uploadedFiles?.[0]?.id);
    ...
    console.log('📅 Creating new activity:', payload);
    ...
    if (!response.ok) {
      ...
      console.error('❌ Activity creation failed with details:', errorText);
...
export async function updateActivity(activityId: number | string, updateData: Partial<{
  ...
  try {
    ...
    if (updateData.file) {
      ...
      console.log('🔥 RAW UPLOAD: Using raw-upload endpoint for activity update...');
      const fileResponse = await fetch(`${STRAPI_BASE_URL}/api/raw-upload`, {
        method: 'POST',
        body: fileFormData,
      });
    ...
    console.log('📝 Updating activity with ID:', activityId, payload);
    ...
    if (response.status === 404 && typeof activityId === 'number') {
      console.log('🔄 Retrying update with numeric ID fallback...');
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ Activity update failed with details:', errorText);
...
export async function deleteActivity(activityId: number | string): Promise<boolean> {
  try {
    ...
    // If 404, try with numeric ID as fallback
    if (response.status === 404 && typeof activityId === 'number') {
      console.log('🔄 Retrying delete with numeric ID fallback...');
      response = await fetch(`${STRAPI_BASE_URL}/api/activities/${activityId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ Activity delete failed with details:', errorText);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return [
        {
          id: 1,
          title: 'Rímske pomocné jednotky v Panónii',
          subtitle: 'Štruktúra a organizácia auxilií',
          content: 'Pomocné jednotky hrali kľúčovú úlohu v rímskej armáde...',
          excerpt: 'Objavte fascinujúci svet rímskych pomocných jednotiek a ich význam pre rímsku armádu.',
          ...
          tags: ['pomocné jednotky', 'panónia', 'auxilium'],
        },
        {
          id: 2,
          title: 'Vzbrojenie a výzbroj pomocných jednotiek',
          subtitle: 'Technická analýza historických nálezov',
          content: 'Archeologické nálezy nám poskytujú detailný obraz...',
          excerpt: 'Analýza archeologických nálezov vzbrojenia a výzbroje rímskych pomocných jednotiek.',
          ...
          tags: ['vzbrojenie', 'archeológia', 'výzbroj'],
        }
      ];
    }
    ...
    const processedArticles = await Promise.all(
      filteredArticles.map(async (item: StrapiHistoryArticle): Promise<HistoryArticleData> => {
        ...
        return {
          ...
          excerpt: item.excerpt || (typeof item.intro === 'string' ? item.intro.substring(0, 200) + '...' : '') || (typeof item.mainContent === 'string' ? item.mainContent.substring(0, 200) + '...' : '') || item.description || 'No excerpt available',
          slug: item.slug,
    ...
    return [
      {
        id: 1,
        title: 'XV. légia Apollinaris na Dunaji',
        subtitle: 'História a pôsobenie légií v naších krajinách',
        content: 'XV. légia Apollinaris bola jedna z najvýznamnejších rímskych légií...',
        excerpt: 'Spoznajte históriu XV. légie Apollinaris a jej pôsobenie na území dnešného Slovenska.',
        ...
        tags: ['légia', 'dunaj', 'apollinaris'],
      },
      {
        id: 2,
        title: 'Rímsky vojenský tábor v Gerulata',
        subtitle: 'Archeologické nálezy z Rusoviec',
        content: 'Gerulata bola významným rímskym vojenským táborom...',
        excerpt: 'Objavte tajomstvá rímskeho vojenského tábora Gerulata na území dnešných Rusoviec.',
        ...
        tags: ['gerulata', 'archeológia', 'rusovce'],
      }
    ];
  }
}
      ...
      contentBlocks: Array.isArray(item.contentBlocks) ? await (async () => {
        ...
        try {
          ...
          if (mediaResponse.ok) {
            ...
            console.log(`📸 Loaded ${allMedia.length} media files for contentBlocks`);
          }
        } catch (error) {
        ...
        return item.contentBlocks.map(block => {
          console.log('🔍 Processing contentBlock:');
          ...
          if (block.__component === 'content.floating-text-image' || block.__component === 'article.content-image') {
            ...
            if (imageId && typeof imageId === 'number' && !block.image?.url) {
              ...
              if (mediaFile) {
                console.log(`✅ Populating image from ID: ${imageId}, found:`, mediaFile.name);
                block.image = {
                  id: mediaFile.id,
                ...
                console.warn(`❌ No media found for ID: ${imageId}`);
              }
            }
            // If image is already an object but missing URL
              ...
              if (mediaFile) {
                console.log(`✅ Fixing image object for ID: ${block.image.id}`);
                block.image = {
                  ...block.image,
                  url: mediaFile.url.startsWith('http') ?
                    mediaFile.url : `${STRAPI_BASE_URL}${mediaFile.url}`,
...
// Helper function to upload image via raw-upload
async function uploadImageRaw(file: File, fieldName: string = 'image'): Promise<number | null> {
  try {
    const fileFormData = new FormData();
    fileFormData.append('files', file);

    console.log(`🔥 RAW UPLOAD for ${fieldName}: File size =`, file.size, 'name =', file.name);
    const fileResponse = await fetch(`${STRAPI_BASE_URL}/api/raw-upload`, {
      method: 'POST',
      body: fileFormData,
    });
    ...
    const uploadedFiles = await fileResponse.json();
    console.log(`🎯 ${fieldName} uploaded SUCCESS:`, uploadedFiles?.length, 'files, first ID =', uploadedFiles?.[0]?.id);
...
// Create History Article - NEW VERSION with multiple image support
export async function createHistoryArticle(articleData: {
  ...
  try {
    // Step 1: Upload all images via raw-upload (prevents rotation)
    let coverImageId = null;
    let heroImageId = null;
    let mainImageId = null;
    const additionalImageIds = [];

    if (articleData.coverImageFile) {
      coverImageId = await uploadImageRaw(articleData.coverImageFile, 'coverImage');
    }

    if (articleData.heroImageFile) {
      heroImageId = await uploadImageRaw(articleData.heroImageFile, 'heroImage');
    }

    if (articleData.mainImageFile) {
      mainImageId = await uploadImageRaw(articleData.mainImageFile, 'mainImage');
    }

    // Upload additional images if provided
    if (articleData.additionalImages && articleData.additionalImages.length > 0) {
      for (const imgFile of articleData.additionalImages) {
        const imgId = await uploadImageRaw(imgFile, 'additionalImage');
        if (imgId) additionalImageIds.push(imgId);
      }
    }
    ...
    const payload = {
      data: {
        ...
        description: articleData.subtitle || articleData.excerpt || articleData.title,
        mainContent: articleData.content,
        intro: articleData.excerpt || (articleData.content ? articleData.content.substring(0, 200) : ''),
        category: articleData.category,
        author: articleData.author || 'SCEAR',
        readTime: articleData.readingTime || 5,
        featured: articleData.featured || false,
        publishedDate: new Date().toISOString().split('T')[0],
        tags: articleData.tags || [],
        // Link all uploaded images by their IDs
        ...(coverImageId && { coverImage: coverImageId }),
        ...(heroImageId && { heroImage: heroImageId }),
        ...(mainImageId && { mainImage: mainImageId }),
        // Add structured content if provided
        ...(articleData.sections && { sections: articleData.sections }),
        ...(articleData.keyFacts && { keyFacts: articleData.keyFacts }),
        ...(articleData.timeline && { timeline: articleData.timeline }),
        ...(articleData.unitTypes && { unitTypes: articleData.unitTypes }),
        // Store additional images using component structure (like gallery)
        ...(additionalImageIds.length > 0 && {
          images: additionalImageIds.map((id, index) => ({
            image: id,  // Direct ID reference like gallery's photo field
    ...
    console.log('📝 Creating new history article with images:', payload);
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ History article creation failed with details:', errorText);
      throw new Error(`History article creation failed! status: ${response.status}`);
    }

    const result = await response.json();
    console.log('✅ History article created successfully with all images');
    return result.data;
  } catch (error) {
...
// Update History Article - NEW VERSION with multiple image support
export async function updateHistoryArticle(articleId: number | string, updateData: Partial<{
  ...
  existingCoverImageId?: number;
  existingHeroImageId?: number;
  existingMainImageId?: number;
}>): Promise<StrapiHistoryArticle | null> {
  try {
    // Step 1: Upload any new images via raw-upload
    let coverImageId = updateData.existingCoverImageId;
    let heroImageId = updateData.existingHeroImageId;
    let mainImageId = updateData.existingMainImageId;
    const additionalImageIds = [];

    if (updateData.coverImageFile) {
      coverImageId = await uploadImageRaw(updateData.coverImageFile, 'coverImage');
    }

    if (updateData.heroImageFile) {
      heroImageId = await uploadImageRaw(updateData.heroImageFile, 'heroImage');
    }

    if (updateData.mainImageFile) {
      mainImageId = await uploadImageRaw(updateData.mainImageFile, 'mainImage');
    }

    // Upload additional images if provided
    if (updateData.additionalImages && updateData.additionalImages.length > 0) {
      for (const imgFile of updateData.additionalImages) {
        const imgId = await uploadImageRaw(imgFile, 'additionalImage');
        if (imgId) additionalImageIds.push(imgId);
      }
    }
    ...
    const payload: any = {
      ...
      ...(updateData.subtitle && { description: updateData.subtitle }),
      ...(updateData.content && { mainContent: updateData.content }),
      ...(updateData.excerpt && { intro: updateData.excerpt }),
      ...(updateData.category && { category: updateData.category }),
      ...(updateData.author && { author: updateData.author }),
      ...(updateData.readingTime && { readTime: updateData.readingTime }),
      ...(updateData.featured !== undefined && { featured: updateData.featured }),
      ...(updateData.tags && { tags: updateData.tags }),
      // Update images only if new ones were uploaded or explicitly set
      ...
      // Update structured content if provided
      ...(updateData.sections && { sections: updateData.sections }),
      ...(updateData.keyFacts && { keyFacts: updateData.keyFacts }),
      ...(updateData.timeline && { timeline: updateData.timeline }),
      ...(updateData.unitTypes && { unitTypes: updateData.unitTypes }),
      // Store additional images using component structure (like gallery)
      ...(additionalImageIds.length > 0 && {
        images: additionalImageIds.map((id, index) => ({
          image: id,  // Direct ID reference like gallery's photo field
    ...
    delete payload.existingCoverImageId;
    delete payload.existingHeroImageId;
    delete payload.existingMainImageId;

    console.log('📝 Updating history article with images:', articleId, payload);
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('❌ History article update failed with details:', errorText);
      throw new Error(`Update failed! status: ${response.status}`);
    }

    const result = await response.json();
    console.log('✅ History article updated successfully with all images');
    return result.data;
  } catch (error) {
...
export async function deleteHistoryArticle(articleId: number | string): Promise<boolean> {
  try {
    console.log('🗑️ Deleting history article:', articleId);
    const response = await fetch(`${STRAPI_BASE_URL}/api/history-articles/${articleId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
    ...
    console.log('✅ History article deleted successfully');
    return true;
  } catch (error) {
